import { createParamDecorator } from "./internal"
...
export const Cors = (corsConf: CrossOriginConf) => createParamDecorator(({ key, index }, ctx) => {
    const { addons } = ctx.method(key).value

    addons.set(Cors, Object.assign({}, defaultCrossOriginConf, corsConf))
})

export const CorsFilter = (corsFilterConf: CrossOriginFilterConf) => createParamDecorator(({ key, index }, ctx) => {
    const { addons } = ctx.method(key).value
